...
func (f *FormField) updateValue(id, val string, res map[string]interface{}, typ PostType, sql *db.SQL) *FormField {
    ...
    if f.isBelongToATable() {
        ...
        if f.FormType.IsSelect() {
            ...
            // 修复select hidden时没有value的bug
            if f.Value == `` {
                f.Value = template.HTML(val)
            }
...
`, m["theme"], m["font_size"], m["language"], m["options"]))
    ...
    if f.FieldList[f.curFieldListIndex].OptionExt != `` {
        ...
        if jsonErr == nil {
            ...
            newOptJson, _ = json.Marshal(oldOpt)
        }
...
type (
    FormPreProcessFn  func(v form.Values) form.Values
    FormPostFn        func(v form.Values) error
