...
func (eng *Engine) Use(router interface{}) error {
    ...
    err := eng.Adapter.Use(router, eng.PluginList)
    return err
}
...
func emptyAdapterPanic() {
    logger.Panic(language.Get("adapter is nil, import the default adapter or use add adapter method add the adapter"))
}
...
func (eng *Engine) deferHandler(_ db.Connection) context.Handler {
    return func(ctx *context.Context) {
        defer func(ctx *context.Context) {
            /*如果需要全局after钩子,可以在这里加入*/
...
// wrapWithAuthMiddleware wrap an auth middleware to the given handler.
...
func (eng *Engine) AddNavButtonsRaw(buttons ...types.Button) *Engine {
    *eng.NavButtons = append(*eng.NavButtons, buttons...)
    return eng
}
...
func (eng *Engine) HTML(method, url string, fn types.GetPanelInfoFn, noAuth ...bool) {
    ...
    var handler = func(ctx *context.Context) {
        ...
        if hasError != nil {
            logger.Error(fmt.Sprintf("error: %s adapter content, ", eng.Adapter.Name()), hasError, tmplName)
        }
...
func (eng *Engine) HTMLFile(method, url, path string, data map[string]interface{}, noAuth ...bool) {
    ...
    var handler = func(ctx *context.Context) {
        ...
        if err != nil {
            eng.errorPanelHTML(ctx, cBuff, err)
            return
        } else if err := t.Execute(cBuff, data); err != nil {
            eng.errorPanelHTML(ctx, cBuff, err)
            return
        }
        ...
        hasError := tmpl.ExecuteTemplate(buf, tmplName, types.NewPage(&types.NewPageParam{
            ...
            Panel: types.Panel{
                Content: template.HTML(cBuff.String()),
            },
            Assets:       template.GetComponentAssetImportHTML(),
            Buttons:      eng.NavButtons.CheckPermission(user),
            TmplHeadHTML: template.Default().GetHeadHTML(),
