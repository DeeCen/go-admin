...
// Add to add a Database to the DatabaseList.
...
type Config struct {
    // A map supports multi database connection. The first
...
// PageAnimation see more: https://daneden.github.io/animate.css/
...
// IsTestEnvironment check the environment if test.
...
func ReadFromJson(path string) (cfg *Config) {
...
func ReadFromYaml(path string) (cfg *Config) {
    jsonByte, err := os.ReadFile(path)
...
func ReadFromINI(path string) (cfg *Config) {
...
func SetDefault(cfg *Config) *Config {
    ...
    cfg.AuthUserTable = utils.SetDefault(cfg.AuthUserTable, "", "goadmin_user")
