...
func SetCookie(ctx *context.Context, user models.UserModel) error {
    ses, err := InitSession(ctx)
    if err != nil {
        return err
    }

    return ses.Add("userId", user.ID)
}

// DelCookie delete the cookie from Context.
func DelCookie(ctx *context.Context) error {
    ses, err := InitSession(ctx)
...
// return true.
func (s *TokenService) CheckToken(_ string) bool {
    return true
