...
func GetUserTable(_ *context.Context) (userTable table.Table) {
    ...
    info.AddColumnButtons("see more", types.GetColumnButton("see more", icon.Info,
        action.PopUp("/see/more/example", "see more", func(ctx *context.Context) (success bool, msg string, data interface{}) {
            return true, "ok", "<h1>Detail</h1><p>bala bala</p>"
        })))
    ...
    info.AddField("createAt", "createAt", db.Timestamp).
        FieldFilterable(types.FilterType{FormType: form.DatetimeRange})
    info.AddField("updateAt", "updateAt", db.Timestamp).FieldEditAble(editType.Datetime)
    ...
    info.AddButton("form", icon.Folder, action.PopUpWithForm(action.PopUpData{
        ...
        panel.AddField("HomePage", "homepage", db.Varchar, form.Url).FieldDefault("https://google.com")
    ...
    info.AddButton("formCtx", icon.Folder, action.PopUpWithCtxForm(action.PopUpData{
        ID:     "/admin/popup/form",
        Title:  "Popup Form Example",
        Width:  "900px",
        Height: "430px",
    }, func(_ *context.Context, panel *types.FormPanel) *types.FormPanel {
        panel.AddField("Name", "name", db.Varchar, form.Text)
        panel.AddField("Age", "age", db.Int, form.Number)
        panel.AddField("HomePage", "homepage", db.Varchar, form.Url).FieldDefault("https://google.com")
        panel.AddField("Email", "email", db.Varchar, form.Email).FieldDefault("xxxx@xxx.com")
...
<input type="text" name="{{.Field}}" value="{{.Value}}" class="form-control {{.Field}}" placeholder="please input {{.Head}}">`)
    formList.AddField("City", "city", db.Varchar, form.SelectSingle).
        ...
        {Text: "GZhou", Value: "gZhou"},
        {Text: "ShenZ", Value: "shenZ"},
    })
    ...
    formList.AddField("updateAt", "updateAt", db.Timestamp, form.Default).FieldDisableWhenCreate()
    formList.AddField("createAt", "createAt", db.Timestamp, form.Datetime).FieldDisableWhenCreate()

    userTable.GetForm().SetTabGroups(types.
        NewTabGroups("id", "ip", "name", "gender", "country", "city").
        AddGroup("phone", "role", "createAt", "updateAt")).
