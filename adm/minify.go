...
func jsMinifier(inputDir, outputFile string, hash bool) {
    ...
    for _, name := range files {
        ...
        var minifiedString []byte
        if strings.Index(name, `.min.`) == -1 {
            m := minify.New()
            ...
            minifiedString = append(minifiedString, byte(59)) //add ;
            fmt.Println("js Minify", filePath)
        } else {
            minifiedString = fileTxt
            fmt.Println("js NOT Minify", filePath)
        }

        minifiedString = append(minifiedString, byte(10)) // add \n
