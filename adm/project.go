...
func installProjectTmpl(p Project, cfg *config.Config, cfgFile string, info *dbInfo) {
    ...
    checkError(err)
    checkError(os.WriteFile("./main.go", c, 0644))
    ...
    if p.Orm == "gorm" {
        ...
        checkError(os.WriteFile("./models/base.go", []byte(`package models
...
`), os.ModePerm))
    ...
    checkError(os.WriteFile("./logs/access.log", []byte{}, os.ModePerm))
    checkError(os.WriteFile("./logs/info.log", []byte{}, os.ModePerm))
    checkError(os.WriteFile("./logs/error.log", []byte{}, os.ModePerm))
    checkError(os.WriteFile("./bootstrap.go", []byte(`package main`), os.ModePerm))

    if p.Theme == "sword" {
        checkError(os.WriteFile("./pages/index.go", swordIndexPage, 0644))
    } else {
        checkError(os.WriteFile("./pages/index.go", adminlteIndexPage, 0644))
    }
    ...
    checkError(os.WriteFile("./Makefile", makefileContent, 0644))

    checkError(os.WriteFile("./html/hello.tmpl", []byte(`<div class="hello">
    <h1>{{index . "msg"}}</h1>
</div>
...
`), 0644))
    ...
    checkError(os.WriteFile("./tables/tables.go", []byte(`// This file is generated by GoAdmin CLI adm.
package tables
