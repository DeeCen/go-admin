...
// Copy a new table.Table from origin DefaultTable
...
func (tb *DefaultTable) getAllDataFromDatabase(params parameter.Parameters) (PanelInfo, error) {
    ...
    thead, _, joins := tb.Info.FieldList.GetThead(types.TableInfo{
        Table:      tb.Info.Table,
...
func (tb *DefaultTable) getDataFromDatabase(params parameter.Parameters) (PanelInfo, error) {
    ...
    if len(ids) > 0 {
        ...
        if connection.Name() == db.DriverMssql {
            // %s means: order by field, order by type, fields, table, join table, where, group by
...
/*func toYmd(timestamp string) (ret string) {
    ts, err := strconv.Atoi(timestamp)
    if err != nil {
        return
    }

    ret = time.Unix(int64(ts), 0).Format(`2006-01-02`)
    return
}*/

func toYmdHis(timestamp string) (ret string) {
    ts, err := strconv.Atoi(timestamp)
    if err != nil {
        return
    }
    ret = time.Unix(int64(ts), 0).Format(`2006-01-02 15:03:04`)
    return
}
