...
func (b *Base) InitPlugin(service.List)            {}
...
func (b *Base) ExecuteTmplWithMenuAndNavButtons(ctx *context.Context, panel types.Panel, _ *menu.Menu,
    ...
    if btns == nil {
        btns = []*types.NavDropDownItemButton{
            ...
            types.GetDropDownItemButton(language.GetFromHtml("menus manage"),
                action.Jump(config.Url("/menu?__pluginName="+name))),
        }
    } else {
